...
tOCTVOCSAMPLES_APP_CFG g_AppCfg =
{
    ...
    { 0x00, 0x0c, 0x90, 0x33, 0x2b, 0x9a },                            /* Vocallo control port's MAC address (port 0 or 1). */
    //{ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },                            /* Vocallo control port's MAC address (port 0 or 1). */
    /* Side A's settings. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80040, 0, 0, 0 } }, 49860 }, /* Host A's RTP UDP address [192.168.0.100:49152]. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80040, 0, 0, 0 } }, 49861 }, /* Host A's RTCP UDP address [192.168.0.100:49153]. */
    "host_a@octasic.com",                                              /* Host A's RTCP canonical name. */
    0,                                                                 /* Vocallo host A's Ethernet port (port 0 or 1). */
    { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80049, 0, 0, 0 } },            /* Vocallo host A's IP address [192.168.0.64]. */
    { cOCTVC1_IP_VERSION_ENUM_4, { 0xFFFFFF00, 0, 0, 0 } },            /* Vocallo host A's network mask [255.255.255.0]. */
    49860,                                                             /* RTP member A's RTP UDP port. */
    49861,                                                             /* RTP member A's RTCP UDP port. */
    ...
    "rtp_member_a1@octasic.com",                                       /* RTP member A's RTCP canonical name. */
    /* Side B's settings. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80A40, 0, 0, 0 } }, 49860 }, /* Host B's RTP UDP address [192.168.10.100:49152]. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80A40, 0, 0, 0 } }, 49861 }, /* Host B's RTCP UDP address [192.168.10.100:49153]. */
    "host_b@octasic.com",                                              /* Host B's RTCP canonical name. */
    0,                                                                 /* Vocallo host B's Ethernet port (port 0 or 1). */
    { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80A49, 0, 0, 0 } },            /* Vocallo host B's IP address [192.168.10.64]. */
    { cOCTVC1_IP_VERSION_ENUM_4, { 0xFFFFFF00, 0, 0, 0 } },            /* Vocallo host B's network mask [255.255.255.0]. */
    49860,                                                             /* RTP member B's RTP UDP port. */
    49861,                                                             /* RTP member B's RTCP UDP port. */
    ...
    "rtp_member_b1@octasic.com"                                        /* RTP member B's RTCP canonical name. */
};

tOCT_UINT8 szBufferName[cOCTVOCSAMPLES_PCM][cOCTVC1_MAIN_FILE_NAME_MAX_LENGTH+1]={
    "../../common/media/audio/35voice_65sil_2_BE.pcm"
};
 ...
 *
 * Description:    Set MC profile for side B.
 *
 * \*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/
...
{
    {
        ...
        StartPlayoutCmd.ulPlayoutDirection = cOCTVC1_VSPMP_VOC_PLAYOUT_DIR_ENUM_RX;
...
int main(int argc, char *argv[])
{
    ...
    ReadTapBuffer("GEN1TAP.bin", g_AppCtx.hVocTermA[15]);
