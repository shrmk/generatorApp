...
#include "octvocsamples_cpp_api_stats.h"
...
tOCTVOCSAMPLES_APP_CFG g_AppCfg =
{
    ...
    96,                                                                 /* RTP member A's RTP payload type for SID */
    ...
    /* Side B's settings. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80A48, 0, 0, 0 } }, 49152 }, /* Host B's RTP UDP address [192.168.10.100:49152]. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80A48, 0, 0, 0 } }, 49153 }, /* Host B's RTCP UDP address [192.168.10.100:49153]. */
    ...
    96,                                                                 /* RTP member B's RTP payload type for SID */
...
static int ProfileIndexA(void);
static int ProfileIndexB(void);
...
Function:       ProfileIndexA

Description:    Set MC profile for side A.
...
static int ProfileIndexA(void)
{
    ...
    \*************************************************************************/
    {
        ...
        ModuleModifyProfileMcCmd.McProfile.ulRxJitterInitialDelay                     = 640;
        ModuleModifyProfileMcCmd.McProfile.ulRxCngFlag                                = cOCT_TRUE;
...
static int CreateSideA(tOCT_UINT32 ulConnectionNumber)
{
    ...
    \*************************************************************************/
    {
        ...
        TermOpenCmd.ulTxEncdrRate           = 0x00000000;
        TermOpenCmd.ulTxOutPktSize          = 0x000000a0;
    ...
    \*************************************************************************/
    {
        ...
        if (cOCTVC1_RC_OK != ulResult)
        {            
            fprintf(stderr, "Error: cOCTVC1_NET_MSG_RTP_SESSION_OPEN_CID failed (Side A) for connection: %d, rc = 0x%08x\n", ulConnectionNumber, ulResult);
            fprintf(stderr, "(%s)\n\n", octvc1_rc2string(ulResult));
            goto ErrorHandling;
        }
...
Function:       ProfileIndexB

Description:    Creates MC Profile for Side B.
...
static int CreateSideB(tOCT_UINT32 ulConnectionNumber)
{
    ...
    \*************************************************************************/
    {
        ...
        if (cOCTVC1_RC_OK != ulResult)
        {            
            fprintf(stderr, "Error: cOCTVC1_NET_MSG_RTP_SESSION_OPEN_CID failed (Side B) for connection:%d, rc = 0x%08x\n", ulConnectionNumber,ulResult);
            fprintf(stderr, "(%s)\n\n", octvc1_rc2string(ulResult));
            goto ErrorHandling;
        }
...
Function:       StartTapBuffer

Description:    Starts recording the tap buffers on the voice termination.

\*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/

static int StartTapBuffer(tOCTVC1_HANDLE f_hVocTerm)
{
    /*************************************************************************\
     * Start the recording of the tap buffer.
    \*************************************************************************/
    {
        tOCTVC1_VSPMP_VOC_MSG_TERM_START_TAP_CMD   VocTermTapStartCmd;
        tOCTVC1_VSPMP_VOC_MSG_TERM_START_TAP_RSP   VocTermTapStartRsp;
        tOCTVC1_PKT_API_CMD_EXECUTE_PARMS          CmdExecute;
        tOCT_UINT32                                ulResult;

        mOCTVC1_VSPMP_VOC_MSG_TERM_START_TAP_CMD_DEF(&VocTermTapStartCmd);
        VocTermTapStartCmd.hTerm            = f_hVocTerm;
        VocTermTapStartCmd.ulTapDataMask    = cOCTVC1_VSPMP_VOC_TAP_DATA_MASK_TX_INPUT; 
        VocTermTapStartCmd.ulDurationMs     = 30000;
        VocTermTapStartCmd.ulWrapEnableFlag = cOCT_FALSE;
        mOCTVC1_VSPMP_VOC_MSG_TERM_START_TAP_CMD_SWAP(&VocTermTapStartCmd);

        /*
         * Execute the command.
         */
        mOCTVC1_PKT_API_CMD_EXECUTE_PARMS_DEF(&CmdExecute);
        CmdExecute.pCmd           = &VocTermTapStartCmd;
        CmdExecute.pRsp           = &VocTermTapStartRsp;
        CmdExecute.ulMaxRspLength = sizeof(VocTermTapStartRsp);
        ulResult = OctVc1PktApiSessCmdExecute(g_AppCtx.PktApiInfo.pPktApiSess, &CmdExecute);
        if (cOCTVC1_RC_OK != ulResult)
        {
            fprintf(stderr, "Error: cOCTVC1_VSPMP_VOC_MSG_TERM_START_TAP_CID failed, rc = 0x%08x\n", ulResult);
            fprintf(stderr, "(%s)\n\n", octvc1_rc2string(ulResult));
            goto ErrorHandling;
...
Function:       StopTapBuffer

Description:    Stops the prompt buffer that is currently playing.

\*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/
static int StopTapBuffer(tOCTVC1_HANDLE f_hVocTerm)
{
    /*************************************************************************\
     * Stop the recording of the tap buffer.
    \*************************************************************************/
    {
        tOCTVC1_VSPMP_VOC_MSG_TERM_STOP_TAP_CMD VocTermTapStopCmd;
        tOCTVC1_VSPMP_VOC_MSG_TERM_STOP_TAP_RSP VocTermTapStopRsp;
        tOCTVC1_PKT_API_CMD_EXECUTE_PARMS       CmdExecute;
        tOCT_UINT32                             ulResult;

        mOCTVC1_VSPMP_VOC_MSG_TERM_STOP_TAP_CMD_DEF(&VocTermTapStopCmd);
        VocTermTapStopCmd.hTerm = f_hVocTerm;
        mOCTVC1_VSPMP_VOC_MSG_TERM_STOP_TAP_CMD_SWAP(&VocTermTapStopCmd);

        /*
         * Execute the command.
         */
        mOCTVC1_PKT_API_CMD_EXECUTE_PARMS_DEF(&CmdExecute);
        CmdExecute.pCmd           = &VocTermTapStopCmd;
        CmdExecute.pRsp           = &VocTermTapStopRsp;
        CmdExecute.ulMaxRspLength = sizeof(VocTermTapStopRsp);
        ulResult = OctVc1PktApiSessCmdExecute(g_AppCtx.PktApiInfo.pPktApiSess, &CmdExecute);
        if (cOCTVC1_RC_OK != ulResult)
        {
            fprintf(stderr, "Error: cOCTVC1_VSPMP_VOC_MSG_TERM_STOP_TAP_CID failed, rc = 0x%08x\n", ulResult);
            fprintf(stderr, "(%s)\n\n", octvc1_rc2string(ulResult));
            goto ErrorHandling;
        }
    }
...
Function:       ReadTapBuffer

Description:    Read the content of the Tap buffer.

\*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/

static void ReadTapBuffer(const char* f_pszTapFileName, tOCTVC1_HANDLE f_hVocTerm)
{
    tOCTVC1_VSPMP_VOC_MSG_TERM_READ_TAP_CMD VocTermTapReadCmd;
    tOCTVC1_VSPMP_VOC_MSG_TERM_READ_TAP_RSP VocTermTapReadRsp;
    tOCTVC1_PKT_API_CMD_EXECUTE_PARMS       CmdExecute;
    tOCT_UINT32                             ulResult;
    FILE*                                    pfTapData;

    pfTapData = fopen(f_pszTapFileName, "wb");

    do
    {
        mOCTVC1_VSPMP_VOC_MSG_TERM_READ_TAP_CMD_DEF(&VocTermTapReadCmd);
        VocTermTapReadCmd.hTerm            = f_hVocTerm;
        VocTermTapReadCmd.ulReadByteOffset = ftell(pfTapData);
        mOCTVC1_VSPMP_VOC_MSG_TERM_READ_TAP_CMD_SWAP(&VocTermTapReadCmd);

        /*
         * Execute the command.
         */
        mOCTVC1_PKT_API_CMD_EXECUTE_PARMS_DEF(&CmdExecute);
        CmdExecute.pCmd           = &VocTermTapReadCmd;
        CmdExecute.pRsp           = &VocTermTapReadRsp;
        CmdExecute.ulMaxRspLength = sizeof(VocTermTapReadRsp);
        ulResult = OctVc1PktApiSessCmdExecute(g_AppCtx.PktApiInfo.pPktApiSess, &CmdExecute);
        if (cOCTVC1_RC_OK != ulResult)
        {
            fprintf(stderr, "Error: cOCTVC1_VSPMP_VOC_MSG_TERM_READ_TAP_CID failed, rc = 0x%08x\n", ulResult);
            fprintf(stderr, "(%s)\n\n", octvc1_rc2string(ulResult));
            break;
        }

        mOCTVC1_VSPMP_VOC_MSG_TERM_READ_TAP_RSP_SWAP(&VocTermTapReadRsp);

        if(0 != VocTermTapReadRsp.ulNumBytesRead)
        {
            fwrite(VocTermTapReadRsp.abyReadData, 1, VocTermTapReadRsp.ulNumBytesRead, pfTapData);
        }

    }while(!VocTermTapReadRsp.ulEofFlag);
...
int main(int argc, char *argv[])
{
    ...
    {
        if (ulConnectionNumber == ulNumConnections/2)
        {
            //printf("SideA Remote IP Address %08x\n", g_AppCfg.HostRtpUdpAddressA.IpAddress.aulIpAddress[0]);
            g_AppCfg.HostRtpUdpAddressA.IpAddress.aulIpAddress[0]++;/* Destination RTP UDP IP. */
            g_AppCfg.HostRtcpUdpAddressA.IpAddress.aulIpAddress[0]++;/* Destination RTP UDP IP. */
            g_AppCfg.HostRtpUdpAddressB.IpAddress.aulIpAddress[0]++;/* Destination RTP UDP IP. */
            g_AppCfg.HostRtcpUdpAddressB.IpAddress.aulIpAddress[0]++;/* Destination RTP UDP IP. */
        }            
//        printf("ulConnectionNumber %d\n:",ulConnectionNumber);
//        printf("ulNumConnections %d\n", ulNumConnections);
  //      printf("---------------------------------------\n");
        ...
        (0 != ConnectTerms(ulConnectionNumber)))
      {
        ...
        //if (ulConnectionNumber != 0 && ulConnectionNumber % 100 == 0)
    ...
    /*
     * Start tapping on one termination 
     */
     
     /* StartTapBuffer(g_AppCtx.hVocTermA[15]); */
    ...
    /*
     * Prints the global Ethernet statistics. 
     */
    ulResult = OctVocSamplesPrintModuleGlobalStats(g_AppCtx.PktApiInfo.pPktApiSess);
    if (cOCTVC1_RC_OK != ulResult)
    {
        fprintf(stderr, "Error: OctVocSamplesPrintModuleGlobalStats() failed, rc = 0x%08x\n", ulResult);
        fprintf(stderr, "(%s)\n\n", octvc1_rc2string(ulResult));
        return -1;
    }

    /*
     * Stop tapping on one termination 
     */
     
    // StopTapBuffer(g_AppCtx.hVocTermA[15]);
    ...
    for (ulConnectionNumber=0;ulConnectionNumber<ulNumConnections;ulConnectionNumber++)
        {
            ...
            printf("| RxJitterClkDriftingDelta         : %u (in 125 us)\n", g_AppStats.lRxJitterClkDriftingDelta);
