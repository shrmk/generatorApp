...
#define cOCTVOCSAMPLES_uLAW_PCM 1
#define cOCTVOCSAMPLES_aLAW_PCM 1
...
typedef struct tOCTVOCSAMPLES_APP_CTX_TAG
{
    ...
    tOCTVC1_HANDLE_OBJECT       hBufferNumU[cOCTVOCSAMPLES_uLAW_PCM];
    tOCTVC1_HANDLE_OBJECT       hBufferNumA[cOCTVOCSAMPLES_aLAW_PCM];
...
tOCTVOCSAMPLES_APP_CFG g_AppCfg =
{
    ...
    /* Side A's settings. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80040, 0, 0, 0 } }, 49354 }, /* Host A's RTP UDP address [192.168.0.100:49152]. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80040, 0, 0, 0 } }, 49355 }, /* Host A's RTCP UDP address [192.168.0.100:49153]. */
    ...
    49354,                                                             /* RTP member A's RTP UDP port. */
    49355,                                                             /* RTP member A's RTCP UDP port. */
    ...
    /* Side B's settings. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80A40, 0, 0, 0 } }, 49354 }, /* Host B's RTP UDP address [192.168.10.100:49152]. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80A40, 0, 0, 0 } }, 49355 }, /* Host B's RTCP UDP address [192.168.10.100:49153]. */
    ...
    49354,                                                             /* RTP member B's RTP UDP port. */
    49355,                                                             /* RTP member B's RTCP UDP port. */
...
tOCT_UINT8 szBufferName_uLaw[cOCTVOCSAMPLES_uLAW_PCM][cOCTVC1_MAIN_FILE_NAME_MAX_LENGTH+1]={
    "../../common/media/audio/VoiceBuffer_ulaw1.pcm"
};

tOCT_UINT8 szBufferName_aLaw[cOCTVOCSAMPLES_aLAW_PCM][cOCTVC1_MAIN_FILE_NAME_MAX_LENGTH+1]={
    "../../common/media/audio/prompt0_alaw.pcm"
};
...
static int CreateBufferSideA(void)
{
    ...
    for ( i = 0; i < cOCTVOCSAMPLES_uLAW_PCM; i++)
    {
        ...
        FILE * pFile;
        tOCT_INT32    lSize;
        pFile = fopen ( szBufferName_uLaw[i] , "rb" );
        if ( pFile == NULL )
        {
         ...
         */
{
            ...
            strcpy(BufferOpenCmd.szBufferName, szBufferName_uLaw[i]);
            ...
            g_AppCtx.hBufferNumU[i] = BufferOpenRsp.hBuffer;
         ...
         */
        {
            ...
            do
            {
                ...
                BufferWriteCmd.hBuffer = g_AppCtx.hBufferNumU[i];
...
Function:       CloseBuffersSideA

Description:    Removes the buffers from memory

\*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/
static int CloseBuffersSideA(void)
{
    tOCT_INT32 i;
    ...
    for ( i = 0; i < cOCTVOCSAMPLES_uLAW_PCM; i++)
    {
        ...
        BufferCloseCmd.hBuffer = g_AppCtx.hBufferNumU[i];
...
/*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*\
Function:       CreateBufferSideB

Description:    Creates a playout buffer and loads its content from a
                 specified file.

\*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/
static int CreateBufferSideB(void)
{
    tOCT_INT32 i;

    for ( i = 0; i < cOCTVOCSAMPLES_aLAW_PCM; i++)
    {
        /*
         * Open local file and check size
         */
        FILE * pFile;
        tOCT_INT32    lSize;
        pFile = fopen ( szBufferName_aLaw[i] , "rb" );
        if ( pFile == NULL )
        {
            fprintf(stderr, "Error: Could not open file");
            goto ErrorHandling;
         ...
         */
        {
             ...
             */
            mOCTVC1_MAIN_MSG_BUFFER_OPEN_CMD_DEF(&BufferOpenCmd);
            BufferOpenCmd.ulFormat = cOCTVC1_BUFFER_FORMAT_VSPMP_VOC_ENUM_RAW_G711_PCM_A_LAW;
            BufferOpenCmd.ulByteLength = lSize;
            strcpy(BufferOpenCmd.szBufferName, szBufferName_aLaw[i]);
            mOCTVC1_MAIN_MSG_BUFFER_OPEN_CMD_SWAP(&BufferOpenCmd);
            ...
            g_AppCtx.hBufferNumA[i] = BufferOpenRsp.hBuffer;
         ...
         */
        {
            ...
            do
            {
                 ...
                 */
                mOCTVC1_MAIN_MSG_BUFFER_WRITE_CMD_DEF(&BufferWriteCmd);
                BufferWriteCmd.hBuffer = g_AppCtx.hBufferNumA[i];
                BufferWriteCmd.ulWriteByteOffset = lWriteOffset;
                BufferWriteCmd.ulWriteByteLength = (lSize < cOCTVC1_MAIN_BUFFER_MAX_DATA_BYTE_SIZE) ? lSize : cOCTVC1_MAIN_BUFFER_MAX_DATA_BYTE_SIZE;
                if ( fread(BufferWriteCmd.abyWriteData, sizeof(tOCT_UINT8), BufferWriteCmd.ulWriteByteLength, pFile) != BufferWriteCmd.ulWriteByteLength )
                {
...
Function:       CloseBuffersSideB

Description:    Removes the buffers from memory

\*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/
static int CloseBuffersSideB(void)
{
    tOCT_INT32 i;
    ...
    for ( i = 0; i < cOCTVOCSAMPLES_aLAW_PCM; i++)
    {
         ...
         */
        mOCTVC1_MAIN_MSG_BUFFER_CLOSE_CMD_DEF(&BufferCloseCmd);
        BufferCloseCmd.hBuffer = g_AppCtx.hBufferNumA[i];
        mOCTVC1_MAIN_MSG_BUFFER_CLOSE_CMD_SWAP(&BufferCloseCmd);
...
Function:       PlayToneSideA

Description:    Plays the tones with playout

\*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/

static int PlayToneSideA(tOCTVC1_HANDLE_OBJECT f_ToneId, tOCT_UINT32 ulConnectionNumber)
{
    {
...
Function:       PlayToneSideB

Description:    Plays the tones with playout

\*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/

static int PlayToneSideB(tOCTVC1_HANDLE_OBJECT f_ToneId, tOCT_UINT32 ulConnectionNumber)
{
    {
        tOCTVC1_VSPMP_VOC_MSG_TERM_START_PLAYOUT_CMD    StartPlayoutCmd;
        tOCTVC1_PKT_API_CMD_SEND_PARMS                  CmdSendParms;
...
int main(int argc, char *argv[])
{
    ...
    if (0 != CreateBufferSideA())
    {
        goto ErrorHandling;
    }
    if (0 != CreateBufferSideB())
    {
        goto ErrorHandling;
    }
	...
	printf("Playing out pcm file on SideA TERM#[%d]\n",ulConnectionNumber);
        for ( i = 0; i < cOCTVOCSAMPLES_uLAW_PCM; i++ )
        {
	    if (0 != PlayToneSideA(g_AppCtx.hBufferNumU[i], ulConnectionNumber))
	    {
		goto ErrorHandling;
	    }
        }
	...
	printf("Playing out pcm file on SideB TERM#[%d]\n",ulConnectionNumber);
        for ( i = 0; i < cOCTVOCSAMPLES_aLAW_PCM; i++ )
        {
	    if (0 != PlayToneSideB(g_AppCtx.hBufferNumA[i], ulConnectionNumber))
	    {
		goto ErrorHandling;
	    }
        }
