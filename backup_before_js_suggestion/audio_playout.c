...
tOCTVOCSAMPLES_APP_CFG g_AppCfg =
{
    ...
    /* Host's settings. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80040, 0, 0, 0 } }, 49862 }, /* Host's RTP UDP address [192.168.0.100:49152]. */
    { { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80040, 0, 0, 0 } }, 49863 }, /* Host's RTCP UDP address [192.168.0.100:49153]. */
    "host@octasic.com",                                                /* Host's RTCP canonical name. */
    /* Vocallo's settings. */
    0,                                                                 /* Vocallo host's Ethernet port (port 0 or 1). */
    { cOCTVC1_IP_VERSION_ENUM_4, { 0xC0A80041, 0, 0, 0 } },            /* Vocallo host's IP address [192.168.0.64]. */
    { cOCTVC1_IP_VERSION_ENUM_4, { 0xFFFFFF00, 0, 0, 0 } },            /* Vocallo host's network mask [255.255.255.0]. */
    49862,                                                             /* RTP member's RTP UDP port. */
    49863,                                                             /* RTP member's RTCP UDP port. */
...
{
    {
        ...
        //StartPlayoutCmd.aPlayoutEntry[0].ulEntryIndex = 0;
        StartPlayoutCmd.aPlayoutEntry[0].ulPlayoutDurationMs = 21000;
         ...
         */
        mOCTVC1_PKT_API_CMD_EXECUTE_PARMS_DEF(&CmdExecuteParms);
        CmdExecuteParms.pCmd           = &StartPlayoutCmd;
...
{  
    tOCT_UINT32 ulNumConnections = 354;
    tOCT_UINT32 ulConnectionNumber;
    /*
...
//    if (( 0 != WaitTermPlayDoneEvent()) )
  ...
  //  {
   ...
   // }
    ...
    for (ulConnectionNumber=0;ulConnectionNumber<ulNumConnections;ulConnectionNumber++)
    {
        if (0 != PlayTone(g_AppCtx.hBufferNum[0], ulConnectionNumber))
        {
            goto ErrorHandling;
        }
    }
